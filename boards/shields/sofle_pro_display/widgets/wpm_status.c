...
LV_FONT_DECLARE(sofle_pro_font_12_roboto_extra_bold);
...
void set_wpm_symbol(lv_obj_t *label, struct wpm_status_state state) {
  ...
  lv_obj_set_style_text_font(label, &sofle_pro_font_12_roboto_extra_bold, LV_PART_MAIN);
}

void wpm_status_update_cb(struct wpm_status_state state) {
  struct zmk_widget_wpm_status *widget;
  SYS_SLIST_FOR_EACH_CONTAINER(&widgets, widget, node) {
    set_wpm_symbol(widget->obj, state);
  }
}
